"""Page for asymmetric stripline FDA simulation."""
# %% Imports
...
from pycem.fda_scenarios import AsymmetricStripline
from pycem.utilities import get_project_root, list_files


# %% Globals
scenario = AsymmetricStripline()
...
offset_input = html.Div(
    [
        dbc.Label("Trace Offset", html_for=f"offset-input-{scenario.name}"),
        dbc.Input(
            type="number",
            id=f"offset-input-{scenario.name}",
            min=0,
            step=0.001,
            value=0.2
        ),
        dbc.FormText(
            "Enter the offset of the trace from the top ground plane in "
            "millimeters.",
            color="secondary"
        ),
    ],
...
form = dbc.Form([width_input, height_input, dy_input, offset_input, er_input,
                 dx_input])
...
@app.long_callback(
    ...
    State(f'offset-input-{scenario.name}', 'value'),
...
def analytical_calc(_, sim_z0, width, height, dy, offset, Er, dx):
