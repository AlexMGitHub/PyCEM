"""Page for coaxial FDA simulation."""
# %% Imports
...
from pycem.fda_scenarios import Coaxial
...
di_input = html.Div(
    [
        dbc.Label("Inner Diameter (Di)",
                  html_for=f"di-input-{scenario.name}"),
        dbc.Input(type="number",
                  id=f"di-input-{scenario.name}",
                  min=0,
                  step=0.001,
                  value=1.3),
        dbc.FormText(
            "Enter the diameter of the inner conductor in millimeters.",
            color="secondary",
        ),
    ],
    className="mb-3",
...
do_input = html.Div(
    [
        dbc.Label("Outer Diameter (Do)",
                  html_for=f"do-input-{scenario.name}"),
        dbc.Input(
            type="number",
            id=f"do-input-{scenario.name}",
            min=0,
            step=0.001,
            value=4.6
        ),
        dbc.FormText(
            "Enter the diameter of the outer conductor in millimeters.",
            color="secondary"
        ),
    ],
    className="mb-3",
...
dy_input = html.Div(
    [
        dbc.Label("Y-direction grid cell size (dy)",
                  html_for=f"dy-input-{scenario.name}"),
        dbc.Input(
            type="number",
        ...
        dbc.FormText(
            "Enter size of the grid cell in the Y-direction (vertical).",
            color="secondary"
        ),
    ],
    className="mb-3",
...
er_input = html.Div(
    [
        ...
        dbc.FormText(
            "Enter the dielectric constant of the insulator material.",
            color="secondary"
        ),
    ],
    className="mb-3",
...
form = dbc.Form([di_input, do_input, dy_input, er_input, dx_input])
...
@app.long_callback(
    ...
    State(f'di-input-{scenario.name}', 'value'),
    State(f'do-input-{scenario.name}', 'value'),
...
def run_sim(set_progress, sim_nclicks, di, do, dy, Er, dx):
    """Run FDA simulation and update table and carousel Dash elements."""
    if sim_nclicks == 0:
        return "Simulation not run!", [diagram_dict], ""
    if di and do and dy and Er and dx:
        scenario.inner_rad = di/2*1e-3
        scenario.outer_rad = do/2*1e-3
        scenario.dy = dy*1e-3
        scenario.Er = Er
        scenario.dx = dx*1e-3
...
def analytical_calc(_, sim_z0, di, do, dy, Er, dx):
